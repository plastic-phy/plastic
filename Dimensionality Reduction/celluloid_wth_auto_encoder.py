...
def cluster_and_output(k, matrix, clust_type, inputpath, outdir):
    ...
    np.savetxt('{}_autoencoder-reducedCells-celluloid.matrix'.format(outfile), out_matrix, fmt='%d', delimiter=' ')

    with open('{}_autoencoder-reducedCells-celluloid_clusters.txt'.format(outfile), 'w+') as file_out:
        for cluster in sorted(cluster_groups):
            file_out.write('{0}\t"{1}"\n'.format(
                cluster, ','.join([ str(x+1) for x in cluster_groups[cluster]])
            ))

    with open('{}_autoencoder-reducedCells-celluloid.mutations'.format(outfile), 'w+') as file_out:
        for cluster in sorted(cluster_groups):
            file_out.write('{0}\n'.format(
...
for i in range(1,51):
    ...
    output_directory = "D:/University/RA/Celluloid/celluloid-master/data/" + exp_name + "/autoencoder-reducedCells-celluloid/"
