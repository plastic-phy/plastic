...
def cluster_and_output(k, matrix, clust_type, inputpath, outdir,num_init_2):
    km = KModes(
    n_clusters = k,
    ...
    np.savetxt('{}_ridge-reducedCells-kmodes.matrix'.format(outfile), out_matrix, fmt='%d', delimiter=' ')

    with open('{}_ridge-reducedCells-kmodes_clusters.txt'.format(outfile), 'w+') as file_out:
        for cluster in sorted(cluster_groups):
            file_out.write('{0}\t"{1}"\n'.format(
                cluster, ','.join([ str(x+1) for x in cluster_groups[cluster]])
            ))

    with open('{}_ridge-reducedCells-kmodes.mutations'.format(outfile), 'w+') as file_out:
        for cluster in sorted(cluster_groups):
            file_out.write('{0}\n'.format(
...
for i in range(1,51):
    ...
    output_directory = "D:/University/RA/Celluloid/celluloid-master/data/" + exp_name + "/ridge-reducedCells-kmodes/"
